...
RWebLogo R Code, wrapper for python code (BSD licence) \cite{rweblogo}
WebLogo Python Code Python Code (BSD license, somewhat difficult to use)
WebLogo 3.0 \cite{weblogo}

Seq2Logo \cite{seq2logo} (Online app. for peptide alignments feat. pseudo count, sequence weighting and two-sided representation)
MoRAine (Online application with integrated binding site re-annotation) \cite{moraine}
GENIO (Online)
