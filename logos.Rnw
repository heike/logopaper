...
\begin{enumerate}
\item We need a gentler intro to logo plots and a lit review of them, including the citations coming from the biovis re-design challenge, and the reference to  seqLogo (bioconductor package by \citet{seqLogo})
...
The data for this re-design challenge contains a set of 1809 sequences (923 Gram-negative and 886 Gram-positive sequences) of 36 amino acids each. 
 ...
 Based on data provided in form of amino acid sequences, we focused on a re-design of the logo plot that will allow to easily answer the following three questions: 
...
\item GENIO (Online, http://www.biogenio.com/logo/ - no other apparent reference)
\item PWM-based logo (Online application for motif PWM-based models) - XXX can't find an appropriate reference
\item LogoBar \citep{logobar} (Java application)
\item CorreLogo \citep{correlogo} An online server for 3D sequence logos of RNA and DNA alignments
\item seqlogo \citep{seqlogo} C function to generate DNA sequence logos (the reference is for the R package. I am not sure that there are C functions)
...
\item RILogo program and web server \citep{rilogo} for creating logos for two interacting RNAs
\item Skylign Online tool  \citep{skylign} for creating logos representing both sequence alignments and profile hidden Markov models
\end{itemize}
